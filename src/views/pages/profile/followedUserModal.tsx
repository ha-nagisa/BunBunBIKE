...
import { getProfileFollowedgUsers } from '../../../utils/firebase';
...
const FollowedUserModal: React.FC<PROPS> = ({ user, setIsOpenFollowedModal }) => {
  const history = useHistory();
  const [followedUsers, setFollowedgUsers] = useState<responceUserData[]>([]);
  ...
  useEffect(() => {
    ...
    if (user?.userId) {
      getFollowedUsers().catch((err) => alert((err as Error).message));
    }
...
export default FollowedUserModal;
